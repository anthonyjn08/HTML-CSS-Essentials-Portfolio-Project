...
* The hero image features a picture of Bryant, his name, Mamba logo and the text Hall of Fame. Having the picture and name staright away gives users a visual aid to the content of the site.
...
* It uses a list structure to display his achievments so they can be read and understood easily.
...
* I added some videos of the highlighs to break up the list and give the users a view into some of these high points.

### **Career Statistics Page**

![Career Statistics Page](docs/screenshots/career-stats-page.jpg)

* This page features content showing Bryants career averages for both the regular season and playoffs in table form so the information is presented in a clear and readable format.
...
* The form keeps the colour theme going with yellow and purple featuring heavily and the background for the form has an image of Bryant's two jersey numbers.

### **Footer**

![Footer](docs/screenshots/footer.jpg)

* The footer has some links to various external sites: Kobe Bryant's Wikipedia page, the Mamba & Mambacita Foundation created in memory of Bryant and his daughter Gianna and the NBA.com Kobe Bryant Tribute page.
* The colours used match the yellow and purple scheme used throughout the site.

### **Features to Implement**

* A standalone gallery page.
* Use of gif images instead of youtube embedded videos.
* A hamburger menu on the nav bar for mobile devices
...
3. **Itended Outcome** - On mobile devices, the youtube videos would shrink to fit the screen.
    *   ***Issue Found*** - 
        * On smaller devices on the career-achievements page, the youtube videos overlapped the page margins set for device resulting in some overflow.
    * ***Solution***
...
* The second error I encountered was also on the Career Achievements page. The embedded code for Youtube videos included the frameborder attribute which is now obsolete and css should be used for this instead. I have removed the frameborder attribute from the html code and added the relevant code to the css styling, this resolved the issue.
...
* For SEO the score was brought down by the nav bar links size, which lighthouse says overlaps. To fix this I could implement a hamburger style menu in the future. This featured across the mobile tests for all pages.
...
* Best practice score was brought down by an issue with an embedded youtube video which I was unfortunately unable to resolve. A quick google search hinted this could be a javascript issue which I unfortunately do not have knowledge of at this point. For some reason this did not affect mobile score.
...
* The vast majority of the content on the home page/index.html content came from the [Kobe Bryant wiki](https://en.wikipedia.org/wiki/Kobe_Bryant) page, although as a fan of Kobe and basketball some of the content was recalled from my own knowledge.
* As with the home page/index.html, the vast majority of content from the career-achievements.html page came from the [Kobe Bryant wiki](https://en.wikipedia.org/wiki/Kobe_Bryant) page with some content from my own knowledge.
...
* For one of my @media queries I used code from Code Institutes Love Running project. This code is found on lines 326 to 336 in the style.css file and is commented above the code. Any code I would've written myself would be exactly the same.
